...
class ExplorerController extends StateNotifier<ExplorerState> {
  ...
  void setSpeed(double speed) { _clock.setSpeed(speed); state = state.copyWith(speed: speed); }
  ...
  void selectPlanet(String id) {
    state = state.copyWith(selectedPlanetId: id, focusedPlanetId: id, detailZoom: 1.0, detailTheta: 0.65, detailPhi: 0.28);
    _checkMissions(id);
  }

  void closeDetail() {
    state = ExplorerState(tab: state.tab, running: state.running, speed: state.speed, showOrbits: state.showOrbits, showLabels: state.showLabels, detailZoom: state.detailZoom, detailTheta: state.detailTheta, detailPhi: state.detailPhi, missions: state.missions, toasts: state.toasts);
  }

  void updateDetailCamera({double? zoom, double? theta, double? phi}) => state = state.copyWith(detailZoom: zoom ?? state.detailZoom, detailTheta: theta ?? state.detailTheta, detailPhi: phi ?? state.detailPhi);

  void runExperiment(String experiment) {
    switch (experiment) {
      case 'sun': selectPlanet('sun'); showToast('Inspecting the blazing Sun'); return;
      case 'earth': selectPlanet('earth'); showToast('Earth experiment started'); return;
      case 'saturn': selectPlanet('saturn'); showToast("Inspect Saturn's rings"); return;
      case 'earth-explore': selectPlanet('earth'); showToast('Explore Earth in 3D'); return;
    }
  }
  ...
  void showToast(String text) {
    ...
    _toastTimer = Timer(const Duration(seconds: 3), () { state = state.copyWith(toasts: state.toasts.where((t) => t.key != key).toList()); });
  }
  ...
  String _planetName(String id) {
    switch (id) {
      case 'sun': return 'the Sun'; case 'mercury': return 'Mercury'; case 'venus': return 'Venus'; case 'earth': return 'Earth';
      case 'mars': return 'Mars'; case 'jupiter': return 'Jupiter'; case 'saturn': return 'Saturn'; case 'uranus': return 'Uranus'; case 'neptune': return 'Neptune';
      default: return id;
    }
  }
  @override void dispose() { _toastTimer?.cancel(); super.dispose(); }
}